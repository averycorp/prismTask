...
**PrismTask** (`com.averycorp.prismtask`) is a native Android todo list app built with Kotlin and Jetpack Compose, with a companion React/TypeScript/Vite web app (`web/`) and a FastAPI backend (`backend/`, Railway-hosted). Currently at v1.9.x (May 2026; baseline below covers everything shipped through v1.9 and reflects the current `main` codebase). Roadmap and ship-status of forward-looking work is tracked in [README.md § Roadmap](README.md#roadmap); per-version detail lives in [CHANGELOG.md](CHANGELOG.md) — post-v1.6 work accumulates in the `Unreleased` section until the next versioned cut.
...
- **Daily Essentials**: `DailyEssentialsUseCase` + `DailyEssentialsPreferences` surface a daily housework + schoolwork card on Today; `housework_habit_id` / `schoolwork_habit_id` point to user-chosen habits and the use case hides the card gracefully when the habit is deleted or archived. (v1.9.x: the Leisure Minimum card was retired from Daily Essentials; Leisure and School now surface as Today-screen modes with per-class checkable rows.)

**v1.7.x — v1.9.x (shipped, accumulating in `CHANGELOG.md#unreleased`):**

- **AI Coach chat hardening**: multi-turn memory (rolling N=6 user/assistant pairs) and task-content forwarded to Anthropic, first-run disclosure V3, action-chip idempotency + per-item batch results, snackbar Undo on destructive chip actions, batch-command inline actions routed through `BatchPreviewScreen`. Conversation persistence (`/api/v1/ai/chat/history` + Postgres + Room mirror via `ChatMessageEntity`/`ChatMessageDao`) is server-authoritative; daily rollover is a UI filter, not a destructive event. Conversation IDs follow the `chat_{ISO_DATE}_{UUID8}` pattern. **No Firestore** for chat — follows the BackendSyncService precedent. Disclosure pref key: `KEY_AI_CHAT_DISCLOSURE_SHOWN_V3`.
- **Claude-backed Auto button on Life Category**: tapping the OrganizeTab "Auto" button next to the Life Category chips fires `POST /ai/life-category/classify_text` on top of the on-device `LifeCategoryClassifier`. Local pick runs first for instant feedback; the AI call upgrades it only when Claude returns a more confident category. Offline / logged-out / AI-features-off / 429 / 451 / 5xx all silently fall back to the local pick — never blanks a successful local chip. Mirrors the Eisenhower text-classify pattern.
- **Time-tracking analytics**: new `TaskTimingEntity` + `TaskTimingDao` + `TaskTimingRepository` capture per-task elapsed time; manual "Logged Time" section in the Schedule tab, Pomodoro auto-log into `task_timings`, cross-device Firestore sync for timings, productivity-score chart + time-tracking bar chart on `TaskAnalyticsScreen` (Pro-gated, `ProductivityScoreCalculator` use case).
- **Today + recurrence**: tap a completed recurring task to choose Log Again or Mark Incomplete; "done" counter opens a sheet listing today's completed tasks + habits; Leisure / School treated as modes with per-class checkable habit-style rows; next-occurrence anchored to completion time (not original due date); toggle-uncomplete rolls back the spawned recurrence; Undo + redo no longer duplicates recurring tasks; Eisenhower / Pomodoro completion spawns recurrences correctly. `MorningCheckInBannerDecider` replaces the hardcoded `HOUR_OF_DAY < 11` check so the banner respects Start-of-Day end-to-end (`MorningCheckInPromptCutoff` slider in Settings → Advanced Tuning).
- **Editor + NLP**: OrganizeTab Auto buttons always assign a chip (and surface feedback when they fall back); auto-classify of task mode + cognitive load runs on every insert; urgency-keyword inference added to NLP; import preview shows due date / priority / tags / subtasks.
- **Projects + tasks UX**: Archive / Reopen actions surfaced from list screens; archived projects hidden from chip rows + project pickers; archived projects moved behind a footer link; Tasks tab elevates project prominence.
- **Medications**: medications can be created without a slot (as-needed / PRN); editor opens with fresh per-med settings and correct slot links; slot-level "Taken at" line dropped in favour of per-med times; the Daily Essentials medication card was removed from Today (start). Per-medication reminder-mode override UI shipped on Android in the v1.7+ window; web equivalents remain in the roadmap.
- **Leisure**: user-defined custom categories alongside the built-in four; built-in defaults are now removable; duration dialog always shown so users can override default minutes; category enable/disable switches no longer hang.
- **Widgets**: timer can be started from the Timer widget without opening the app.
- **Sync hardening (web)**: Firestore-direct write path for `task_completions`; `startOfDayHour` synced cross-device via `users/{uid}/prefs/task_behavior_prefs.day_start_hour`; IndexedDB cache cleared on logout to keep different users on the same browser isolated; 4 of 5 additional real-time listeners wired (`subscribeToDependencies`, `subscribeToPhases`, `subscribeToRisks`, `subscribeToAnchors`) via `useFirestoreSync.ts`; dead `web/src/api/sync.ts` HTTP-sync stub removed.
- **Test infrastructure**: `HiltTestRunner.isAndroidEmulator()` now detects modern AVDs; cross-device-tests CI job drops shell-level retry; auto-update-branch workflow added so feature branches rebase on `main` automatically.
