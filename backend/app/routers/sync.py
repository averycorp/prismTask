...
async def sync_pull(
    ...
    for entity_type, model in ENTITY_MAP.items():
        ...
        if hasattr(model, "user_id"):
            query = query.where(model.__table__.c.user_id == current_user.id)

        if since and hasattr(model, "updated_at"):
            query = query.where(model.__table__.c.updated_at > since)
        elif since and hasattr(model, "created_at"):
            query = query.where(model.__table__.c.created_at > since)
