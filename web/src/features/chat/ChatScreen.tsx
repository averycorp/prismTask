...
function actionLabel(action: ChatActionPayload): string {
  switch (action.type) {
    case 'complete':
...
function actionSignature(action: ChatActionPayload): string | null {
  switch (action.type) {
    case 'reschedule_batch': {
...
function resolveDateIsoForChat(to: string | null | undefined): string | null {
  if (!to) return null;
  const now = new Date();
  ...
  // Else assume YYYY-MM-DD already.
...
async function executeChatAction(
  ...
  switch (action.type) {
    ...
    case 'archive': {
      ...
      // user-visible outcome (Android divergence noted in the audit).
    ...
    case 'create_task': {
      ...
      // Wire-up against creator is in PR-5 — we don't have a no-project
      // createTask path on web today. Surface as a toast for now.
      return {
    ...
    case 'breakdown': {
      ...
      // Subtask insertion requires a parent project_id and is wired in
      // PR-5. Surface a toast acknowledging the suggestion.
      return {
