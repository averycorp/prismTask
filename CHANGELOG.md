...
- **AI Coach chat screen on web (parity Batch 3 D.1a).** New `/chat` route + sidebar entry surfaces the chat surface that has shipped on Android since v1.5 — message list with user/assistant bubbles, action chips (`complete`, `reschedule`, `reschedule_batch`, `archive`, `start_timer` + a no-op preview for `create_task`/`breakdown`), composer with Enter-to-send, welcome card with 4 starter prompts, first-run V3 retention disclosure, Clear-Chat confirm (with don't-ask-again pref), and history reconciliation via `GET /api/v1/ai/chat/history` on mount. Conversation IDs follow the `chat_${ISO_DATE}_${UUID8}` pattern minted client-side; daily rollover keys off `startOfDayHour`. Server is authoritative — no Firestore mirror, REST-only.
...
- fix(web/sync): wire taskDependencies / projectPhases / projectRisks / externalAnchors real-time listeners from useFirestoreSync (parity audit A.1a).
